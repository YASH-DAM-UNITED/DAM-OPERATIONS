...
import {
  ...
  Tags, TrendingDown, TrendingUp, UserRoundCog, UsersRound, Warehouse, XCircle,Sun,Moon,
  Zero
} from "lucide-react";
...
function SearchView({data}){
 ...
 const all=[...(data.daily||[]).map(x=>({...x,schedule:"Daily"})),...(data.weekly||[]).map(x=>({...x,schedule:"Weekly"}))];
 const unique=useMemo(()=>{
   const m=new Map();
   all.forEach(r=>{
     const key=`${r.SKU||""}|${r["Item Name"]||""}|${r.UOM||""}|${r.schedule}`;
     if(!m.has(key))m.set(key,r);
   });
   return [...m.values()];
 },[data]);
 const matches=q.trim()?unique.filter(r=>`${r.SKU} ${r["Item Name"]} ${r.UOM}`.toLowerCase().includes(q.toLowerCase())).slice(0,40):[];
 const picked=unique.filter(r=>selected.includes(`${r.SKU||""}|${r["Item Name"]||""}|${r.UOM||""}|${r.schedule}`));
 const toggle=r=>{
   const key=`${r.SKU||""}|${r["Item Name"]||""}|${r.UOM||""}|${r.schedule}`;
 ...
 const downloadSelected=()=>{
   ...
   downloadWorkbook(buildSelectedItemsTransposedWorkbook(picked,branches,data.date),`Selected_Items_Transposed_${data.date}.xlsx`);
 };
 return <div className="ba-stack">
   <PageHead eyebrow="GLOBAL INVENTORY MULTI-SEARCH" title="Find, select and export stock." text="Daily + Weekly inventory together, matching the Streamlit multi-search operation."/>
   <motion.div className="hero-search search-spring" initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{type:"spring",stiffness:160,damping:18}}>
     <Search/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Search SKU, item name or UOM..."/><kbd>LIVE</kbd>
   </motion.div>
   {q&&<div className="multi-search-results">
     {matches.map((r,i)=>{
       const key=`${r.SKU||""}|${r["Item Name"]||""}|${r.UOM||""}|${r.schedule}`;
       const active=selected.includes(key);
       return <motion.button key={key} className={active?"selected":""} onClick={()=>toggle(r)}
         initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:Math.min(i*.015,.25)}}
         whileHover={{y:-3,scale:1.01}} whileTap={{scale:.98}}>
         <span className="pick-check">{active?"✓":"+"}</span><b>{r.SKU||"—"}</b><span>{r["Item Name"]}</span><small>{r.UOM} · {r.schedule}</small>
       </motion.button>
     })}
   </div>}
   {picked.length>0&&<>
     <section className="selected-strip">
       <div><span className="eyebrow">SELECTED PRODUCTS</span><strong>{picked.length}</strong></div>
       <button className="primary-action" onClick={downloadSelected}><Download/> Selected Items Transposed Excel</button>
     </section>
     <AdminTable rows={picked} branches={branches}/>
   </>}
   {!q&&!picked.length&&<Empty text="Start typing to search across Daily + Weekly inventory."/>}
 </div>
}
...
function buildSelectedItemsTransposedWorkbook(rows,branches,date){
  const wb=XLSX.utils.book_new();
  const selected=(rows||[]).filter(Boolean);
  const header1=["Branch"];
  const header2=[""];
  selected.forEach(r=>{
    header1.push(r["Item Name"]||"");
    header1.push("");
    header1.push("");
    header2.push(r.SKU||"");
    header2.push(r.UOM||"");
  ...
  const body=branches.map(branch=>{
    ...
    selected.forEach(r=>{
      line.push(r.SKU||"");
      line.push(r.UOM||"");
      line.push(n(r[branch]));
    });
    return line;
  });
  const aoa=[
    [`BART SELECTED ITEMS — ${date}`],
    header1,
    header2,
    ...body
  ];
  const ws=XLSX.utils.aoa_to_sheet(aoa);
  const endCol=Math.max(0,header1.length-1);
  ws["!merges"]=[{s:{r:0,c:0},e:{r:0,c:endCol}}];
  let c=1;
  selected.forEach(()=>{
    ws["!merges"].push({s:{r:1,c},e:{r:1,c:c+2}});
    c+=3;
  });
  ws["!cols"]=[{wch:24},...Array(Math.max(0,endCol)).fill(0).map((_,i)=>({wch:i%3===2?12:18}))];
  styleWorksheet(ws,{titleRow:0,headerRow:2,freeze:"B4",autoFilter:false});
