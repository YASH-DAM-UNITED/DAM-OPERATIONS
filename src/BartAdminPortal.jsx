...
function CommandCenter({data,date,setTab}){
  ...
  const zero=all.reduce((a,r)=>a+(data.branches||[]).filter(b=>r.availability?.[b.name]==="AVAILABLE"&&n(r[b.name])===0).length,0);
  const attention=(c.missingDateCount||0)+(c.partialCount||0)+(c.fetchFailedCount||0)+unc;
  return <div className="ba-stack">
    <section className="ba-hero"><div><span className="eyebrow">BART / OPERATIONS COMMAND</span><h1>Good day, Admin.</h1><p>One surface for the signals that deserve management attention on <b>{date}</b>.</p></div><div className="hero-radar"><div className="radar-ring r1"/><div className="radar-ring r2"/><div className="radar-dot"/><strong>{c.completeCount??data.loadedBranchCount??0}</strong><span>DATA COMPLETE</span></div></section>
    <div className="ba-metrics"><Metric label="NETWORK" value={`${data.branchCount||0}`} detail="BART branches" Icon={Building2}/><Metric label="COMPLETE" value={`${c.completeCount||0}/${data.branchCount||0}`} detail="selected-date data" Icon={ClipboardCheck}/><Metric label="TRANSFERS" value={transfers.count||0} detail={`records on ${date}`} Icon={Activity}/><Metric label="ATTENTION" value={attention} detail="real data signals" Icon={AlertTriangle}/></div>
    <div className="ba-grid-2"><section className="ba-panel"><header><div><span className="eyebrow">NETWORK CONDITION</span><h3>Selected-date completeness</h3></div><button onClick={()=>setTab("data-health")}>Open health <ChevronRight size={15}/></button></header><div className="health-bar"><i style={{width:`${data.branchCount?((c.completeCount||0)/data.branchCount)*100:0}%`}}/></div><div className="health-numbers"><b>{c.completeCount||0} complete</b><span>{c.missingDateCount||0} missing · {c.partialCount||0} partial · {c.fetchFailedCount||0} failed</span></div></section>
    <section className="ba-panel"><header><div><span className="eyebrow">DATA SIGNALS</span><h3>Immediate checks</h3></div><button onClick={()=>setTab("exception-center")}>Inspect <ChevronRight size={15}/></button></header><div className="signal-list"><div><AlertTriangle/><span><b>{unc}</b> uncategorized item records</span></div><div><XCircle/><span><b>{fmt(zero)}</b> confirmed zero branch/item cells</span></div><div><Database/><span><b>{c.missingDateCount||0}</b> branches missing selected date</span></div></div></section></div>
...
function DataHealth({data}){const c=data.completeness||{};return <div className="ba-stack"><PageHead eyebrow="SYSTEM" title="Data Health" text="Visibility into the Admin data pipeline without exposing credentials."/><div className="ba-metrics"><Metric label="COMPLETE" value={c.completeCount||0} detail="selected-date complete" Icon={ClipboardCheck}/><Metric label="MISSING DATE" value={c.missingDateCount||0} detail="sheet loaded, date absent" Icon={CalendarDays}/><Metric label="FETCH FAILED" value={c.fetchFailedCount??(data.failedBranches||[]).length} detail="Google read failures" Icon={Database}/><Metric label="SOURCE" value={(data.source||"google").toUpperCase()} detail="current aggregation source" Icon={Activity}/></div><section className="ba-panel"><header><div><span className="eyebrow">BRANCH DATA STATUS</span><h3>Exact selected-date diagnostics</h3></div></header>{(data.branchStatus||[]).map((x,i)=><div className="failure" key={i}><span>{x.status==="COMPLETE"?"✓":"!"}</span><span>{x.name}</span><small>{x.status}{x.matchedHeader?` · ${x.matchedHeader}`:""}{x.error?` · ${x.error}`:""}</small></div>)}</section></div>}
...
function BranchWorkspace({branch,data,onBack}){const status=(data.branchStatus||[]).find(x=>x.name===branch.name);const daily=(data.daily||[]).filter(r=>r.availability?.[branch.name]==="AVAILABLE");const weekly=(data.weekly||[]).filter(r=>r.availability?.[branch.name]==="AVAILABLE");const connected=status?.sheetLoaded!==false;return <div className="branch-workspace"><button className="backline" onClick={onBack}><ArrowLeft/> BART / BRANCHES</button><section className="branch-hero"><div><span className="eyebrow">BRANCH WORKSPACE</span><h1>{branch.name}</h1><p>{branch.code} · Selected inventory snapshot</p></div><div className="branch-health"><i/> {status?.status|| (connected?"DATA CONNECTED":"UNAVAILABLE")}</div></section><div className="ba-metrics"><Metric label="DAILY ITEMS" value={daily.length} detail="available records" Icon={ClipboardCheck}/><Metric label="WEEKLY ITEMS" value={weekly.length} detail="available records" Icon={CalendarDays}/><Metric label="DAILY QTY" value={fmt(daily.reduce((a,r)=>a+n(r[branch.name]),0))} detail="branch snapshot" Icon={Boxes}/><Metric label="WEEKLY QTY" value={fmt(weekly.reduce((a,r)=>a+n(r[branch.name]),0))} detail="branch snapshot" Icon={Warehouse}/></div><section className="ba-panel"><header><div><span className="eyebrow">BRANCH INVENTORY</span><h3>Complete selected-date stock</h3></div></header><div className="branch-items">{[...daily,...weekly].map((r,i)=><div key={i}><b>{r["Item Name"]}</b><span>{r.SKU||"—"} · {r.UOM}</span><strong>{fmt(r[branch.name])}</strong></div>)}</div></section></div>}
